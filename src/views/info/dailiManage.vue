<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.dailiName" placeholder="请输入代理商名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>

      <el-select v-model="listQuery.daili" placeholder="全部代理" clearable style="width: 200px" class="filter-item" @change="handleFilter">
        <el-option v-for="item in allDaili" :key="item.key" :label="item.label" :value="item.key"/>
      </el-select>
      <el-select v-model="listQuery.area" placeholder="全部区域" style="width: 200px" class="filter-item" @change="handleFilter" >
        <el-option v-for="item in allArea" :key="item.key" :label="item.label" :value="item.key"/>
      </el-select>
...
export default {
  ...
  data() {
    return {
      ...
      allDaili: [{ label: '一级代理', key: 1 }, { label: '二级代理', key: 2 }, { label: '三级代理', key: 3 }],
      calendarTypeOptions,
      allArea: [{ label: '待审批', key: '1' }, { label: '准备中', key: '2' }, { label: '进行中', key: '3' }, { label: '暂停中', key: '4' }, { label: '已完成', key: '5' }, { label: '未通过', key: '6' }],
